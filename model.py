...
class Model:
    ...
    def generate_bacterium(self):
        ...
        self.bacteria[0].N+=1
        Cell3 = cell.Cell(X=np.array([Cell2.X[0]+self.bacteria[0].spring_rest_l,Cell2.X[1]]),mass=0.1,ray=self.ray)
        self.bacteria[0].Cells.append(Cell3)
        self.bacteria[0].N+=1

        Cell1 = cell.Cell(X=np.array([2,4]),mass=0.1,ray=self.ray)
        Cell2 = cell.Cell(X=np.array([Cell1.X[0],Cell1.X[1]+self.rest_spring_l]),mass=0.1,ray=self.ray)
        Cell3 = cell.Cell(X=np.array([Cell2.X[0],Cell2.X[1]+self.rest_spring_l]),mass=0.1,ray=self.ray)
        Cell4 = cell.Cell(X=np.array([Cell3.X[0]+self.rest_spring_l,Cell3.X[1]]),mass=0.1,ray=self.ray)
        Cell5 = cell.Cell(X=np.array([Cell4.X[0],Cell4.X[1]+self.rest_spring_l]),mass=0.1,ray=self.ray)
        self.bacteria.append (bact.Bacterium(N=5,Cells=[Cell1,Cell2,Cell3,Cell4,Cell5],Torques=[],l=self.rest_spring_l,theta=self.theta))
